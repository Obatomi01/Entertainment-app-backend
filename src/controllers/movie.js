...
exports.getMovies = (req, res, next) => {
  ...
  const categoryFilter = categoryOfEntertainment
    ? { category: categoryOfEntertainment }
    : {};
  ...
  User.findById(loggedInUser).then((user) => {
    ...
    Movie.find(categoryFilter)
      ...
      .then((movies) => {
        ...
        const tvSeriesCategory = movies.filter(
        ...
        if (year && rating !== undefined && rating !== 'null') {
          otherMovies = otherMovies.filter(
            (movie) => movie.year === year && movie.rating === rating
          );
        } else if (year) {
...
exports.bookmarkMovie = (req, res, next) => {
  ...
  console.log(true);
  if (bookmarkState) {
    User.updateOne({ _id: loggedInUser }, { $addToSet: { bookmarks: id } })
      .then((user) => {
    ...
    User.updateOne({ _id: loggedInUser }, { $pull: { bookmarks: id } })
      .then(() => {
        Movie.findOneAndUpdate(
          {
            _id: id,
